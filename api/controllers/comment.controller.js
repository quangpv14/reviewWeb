...
export const createComment = async (req, res, next) => {
    try {
        ...
        const solvecomment = censorProfanity(content)
        const newComment = new Comment({
            content: solvecomment,
            postId,
            userId,
        });
...
export const getcomments = async (req, res, next) => {
    ...
    try {
        ...
        // Lấy thông tin của post, user cho từng comment
        ...
        res.status(200).json({ commentsWithInfo, totalComments, lastMonthComments });
    } catch (error) {
        next(error);
    }
};
